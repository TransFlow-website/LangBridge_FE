...
export default function Glossary() {
  ...
  const handleEditClick = (term: TermDictionaryResponse) => {
    ...
    setFormData({
      ...
      articleSource: term.articleSource || '',
      articleLink: term.articleLink || '',
      memo: term.memo || '',
    });
  ...
  const handleExport = async () => {
    try {
      ...
      const blob = await termApi.exportTerms(params);
      ...
      // 파일명 생성 (필터 정보 포함)
      let filename = 'glossary_export';
      if (selectedSourceLang && selectedTargetLang) {
        filename += `_${selectedSourceLang}_${selectedTargetLang}`;
      } else if (selectedSourceLang) {
        filename += `_${selectedSourceLang}`;
      } else if (selectedTargetLang) {
        filename += `_${selectedTargetLang}`;
      }
      filename += '.tsv';
      ...
      alert('용어집이 성공적으로 내보내졌습니다.');
    } catch (error) {
      console.error('용어집 내보내기 실패:', error);
      alert('용어집 내보내기에 실패했습니다.');
    }
  };

  // 컬럼 정의 (관리자일 때 추가 컬럼 포함)
                ...
                <div style={{ display: 'flex', gap: '12px' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <select
                      ...
                      required
                    >
                      {languageOptions.filter(opt => opt.value).map((opt) => (
                        <option key={opt.value} value={opt.value}>
                          {opt.label}
                        </option>
                      ))}
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={formData.articleSource}
                    onChange={(e) => setFormData({ ...formData, articleSource: e.target.value })}
                    style={{
                      width: '100%',
                      padding: '8px 12px',
                      border: `1px solid ${colors.border}`,
